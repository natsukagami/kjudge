...
(() => {
    ...
    const showUnanswered = document.getElementById("show-unanswered") as HTMLInputElement | null;
    if (!showUnanswered)
        return;
...
(() => {
    for (const elem of document.getElementsByClassName("premade")) {
        const textarea = elem.parentElement?.getElementsByClassName("form-input")[0] as HTMLTextAreaElement;
        const select = elem as HTMLSelectElement;
        select.addEventListener("change", () => {
            textarea.innerHTML = select.selectedOptions[0].value || textarea.innerHTML;
        })
    }
})();
